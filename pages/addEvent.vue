...
// Cостояние для всплывающего уведомления
...
const uploadImage = async (event) => {
  ...
  try {
    ...
    if (response.error) {
      triggerNotification('Ошибка загрузки файла');
    ...
    triggerNotification('Ошибка при отправке запроса');
    console.error(err)
  }
};
...
const createEvent = async () => {
  ...
  try {
    ...
    if (error.response && error.response._data && error.response._data.error) {
      ...
      triggerNotification('Произошла непредвиденная ошибка');
    }
      ...
      <transition name="fade">
        <div v-if="showDescModal" :class="styles.modal_overlay" @click="closeDescModal">
          <div :class="styles.modal_content" @click.stop>
            ...
            <textarea v-model="tempDesc" placeholder="Начните печатать..." maxlength="200"
              :class="styles.input_textarea"></textarea>
